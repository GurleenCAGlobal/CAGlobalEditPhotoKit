...
extension IndefiniteAnimatedView {
    private func getIndefinteAnimatedLayer() -> CAShapeLayer {
        if self.indefinteAnimatedLayer != nil {
            ...
            if let image = loadImageBundle(named: "angle-mask"), let cgImage = image.cgImage {
                maskLayer.contents = cgImage
                maskLayer.frame = indefinteAnimatedLayer?.bounds ?? .zero
                indefinteAnimatedLayer?.mask = maskLayer
            } else {
                print("Error: Failed to load the mask image from the bundle.")
            }
