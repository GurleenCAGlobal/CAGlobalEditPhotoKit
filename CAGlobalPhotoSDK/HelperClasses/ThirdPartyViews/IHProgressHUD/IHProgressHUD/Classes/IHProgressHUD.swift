...
public class IHProgressHUD : UIView {
    ...
    private override init(frame: CGRect) {
        ...
        if let infoImg = loadImageBundle(named: "info") {
            infoImage = infoImg
        } else {
            print("Error: 'info' image not found in bundle.")
        }

        if let successImg = loadImageBundle(named: "success") {
            successImage = successImg
        } else {
            print("Error: 'success' image not found in bundle.")
        }

        if let errorImg = loadImageBundle(named: "error") {
            errorImage = errorImg
        } else {
            print("Error: 'error' image not found in bundle.")
        }
    ...
    @objc private func positionHUD(_ notification: Notification? = nil) {
        ...
        var orientation: UIInterfaceOrientation = .unknown // Initialize with default
        ...
        if IHProgressHUD.isNotAppExtension {
            ...
            if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene {
                // Get orientation from windowScene
...
//                self.frame = containerView!.frame
    ...
    private func dismissWithDelay(_ delay: TimeInterval, completion: (() -> Void)?) {
        OperationQueue.main.addOperation({ [weak self] in
            ...
            let completionBlock: (() -> Void) = {
                // Check if we really achieved to dismiss the HUD and the alpha values are applied
                if strongSelf.getBackGroundView().alpha == 0.0 {
                    ...
                    // Remove observer to avoid handling orientation changes
                    ...
                    // Update status bar appearance on the root view controller
                    #if os(iOS)
                    if IHProgressHUD.isNotAppExtension {
                        // Use the active scene's key window in iOS 13+
                        if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene {
                            let rootController: UIViewController? = windowScene.windows.first(where: { $0.isKeyWindow })?.rootViewController
                            rootController?.setNeedsStatusBarAppearanceUpdate()
                        }
                    }
                    #endif

                    // Run the completion handler if it's not nil
                    completion?()
                }
            }
    ...
    @objc private func deviceOrientationDidChange(_ notification: Notification) {
        // Adjust the HUD position when the device orientation changes
    ...
    public override func layoutSubviews() {
        ...
        // Call positionHUD to adjust the layout if necessary
        self.positionHUD(nil)
    }
