...
public class AMColorPickerRGBSliderView: XibLioadView, AMColorPicker {
    ...
    @objc func didChange(textField: UITextField) {
        ...
        didSelect(color: UIColor(hexaString: newText, alpha: CGFloat(opacitySlider.value) / 100.0))
    }
    ...
    private func didSelect(color: UIColor) {
        selectedColor = color
        delegate?.colorPicker(self, didSelect: color, opacity: CGFloat(opacitySlider.value) / 100.0)
    }
