...
extension Rotatable where Self: CropperViewController {
    ...
    internal func rotate90degrees(clockwise: Bool = true) {
        ...
        UIView.animate(withDuration: 0.25, delay: 0, options: .curveEaseOut, animations: {
            ...
            let rAxis = self.overlay.cropBoxFrame
            var rect: CGRect = .zero

            let scaleX = self.maxCropRegion.size.width / rAxis.size.height
            let scaleY = self.maxCropRegion.size.height / rAxis.size.width

            let scale = min(scaleX, scaleY)

            rect.size.width = rAxis.size.height * scale
            rect.size.height = rAxis.size.width * scale
...
extension Rotatable where Self: CAEditViewController {
    ...
    internal func rotate90degrees(clockwise: Bool = true) {
        ...
        UIView.animate(withDuration: 0.25, delay: 0, options: .curveEaseOut, animations: {
            ...
            let contentOffset = self.scrollView.contentOffset
            let showingContentCenter = CGPoint(x: contentOffset.x + self.scrollView.bounds.size.width / 2, y: contentOffset.y + self.scrollView.bounds.size.height / 2)
            let showingContentNormalizedCenter = CGPoint(x: showingContentCenter.x / self.mainImageView.width, y: showingContentCenter.y / self.mainImageView.height)

            self.scrollView.bounds = CGRect(x: 0, y: 0, width: width, height: height)
            let zoomScale = self.scrollView.zoomScale * scale
            self.willSetScrollViewZoomScale(zoomScale)
            self.scrollView.zoomScale = zoomScale
            let newContentOffset = CGPoint(x: showingContentNormalizedCenter.x * self.mainImageView.width - self.scrollView.bounds.size.width * 0.5,
                                           y: showingContentNormalizedCenter.y * self.mainImageView.height - self.scrollView.bounds.size.height * 0.5)
            self.scrollView.contentOffset = self.safeContentOffsetForScrollView(newContentOffset)
            self.scrollView.center = scrollViewCenter
        }, completion: { _ in
            self.overlay.cropBoxAlpha = 0
